    ...
    def _get_error_det(self, detections, labels):
        ...
        if x[0].shape[0]:
            ...
            if x[0].shape[0] > 1:
                ...
                match_idx = set(matches[:, 0].astype(np.uint8).tolist())
                no_match = labels[list(gt_idx - match_idx)]
        if y[0].shape[0]:
            matches_wrong = torch.cat((torch.stack(y, 1), iou[y[0], y[1]][:, None]),
                                      1).cpu().numpy()  # [label, detect, iou]
            if y[0].shape[0] > 1:
                matches_wrong = matches_wrong[matches_wrong[:, 2].argsort()[::-1]]
                matches_wrong = matches_wrong[np.unique(matches_wrong[:, 1], return_index=True)[1]]
                # matches_wrong = matches_wrong[matches_wrong[:, 2].argsort()[::-1]]
                matches_wrong = matches_wrong[np.unique(matches_wrong[:, 0], return_index=True)[1]]
                self.error_match = matches_wrong.copy()
                match_wrong = detections[matches_wrong[:, 1]]
        return no_match, match_wrong
        ...
        if i.suffix == '.jpg':
            ...
            if pred.exists():
                ...
                if pred == []:
                    ...
                    pred[:,2:3] = bbox[:, 1:2] + bbox[:, 2:3]
                    pred[:,3:4] = bbox[:, 2:3] + bbox[:, 3:4]
                    pred[:,4:5] = conf
                    pred[:, 5:6] = cl
            else:
                pred = torch.empty([0, 6], dtype=torch.float32)

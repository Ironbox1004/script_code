...
def voc2coco():
    ...
    # 创建coco的文件夹
    if not os.path.exists(os.path.join(root_dir, "coco_val")):
        os.makedirs(os.path.join(root_dir, "coco_val"))
        os.makedirs(os.path.join(root_dir, "coco_val", "annotations"))
        os.makedirs(os.path.join(root_dir, "coco_val", "train"))
        os.makedirs(os.path.join(root_dir, "coco_val", "val"))
    ...
    images_dir = os.path.join(root_dir,  'VOC_DT_20221107', 'JPEGImages')
    ...
    # 获取训练图片
    train_img = []
    fp = open(os.path.join(root_dir, 'VOC_DT_20221107', 'ImageSets', 'Main', 'train.txt'))
    ...
    for xml in train_xml:
        ...
        tree = ET.parse(os.path.join(root_dir,  'VOC_DT_20221107', 'Annotations', xml))
        root = tree.getroot()
        ...
        for i in range(len(category)):
            ...
            bbox_id += 1
    # 生成训练集的json
    json.dump(data, open(os.path.join(root_dir, 'coco_val', 'annotations', 'train.json'), 'w'))

    # 获取验证图片
    val_img = []
    fp = open(os.path.join(root_dir,  'VOC_DT_20221107', 'ImageSets', 'Main', 'test.txt'))
    for line in fp.readlines():
        list = line[:-1]
        file_name = (list.split('/')[-1]).split('.')
        if len(file_name) == 1:
    ...
    for xml in val_xml:
        ...
        for i in range(len(category)):
            ...
            bbox_id += 1
    # 生成验证集的json
    json.dump(data, open(os.path.join(root_dir, 'coco_val', 'annotations', 'val.json'), 'w'))
    ...
    m = len(train_img)
    for i in trange(m):
        shutil.copy(os.path.join(images_dir, train_img[i]), os.path.join(root_dir, 'coco_val', 'train', train_img[i]))
    print('| Train images copy finish.')

    m = len(val_img)
    for i in trange(m):
        shutil.copy(os.path.join(images_dir, val_img[i]), os.path.join(root_dir, 'coco_val', 'val', val_img[i]))
